...
const ASSETS_DIR = path.join(__dirname, "assets", "fakecall-andro");
...
async function generateFakeCallAndroid({ avatarUrl, name, duration }) {
  registerFonts();
  ...
  if (avatarImg) {
    ...
    const ppY = canvas.height * 0.40;
    const ppRadius = canvas.width * 0.255;
    ctx.save();
    ctx.beginPath();
  ...
  await drawTextWithEmojisCenter(ctx, duration || "00:00", durasiY, durasiSize, `400 ${durasiSize}px "RobotoRegular", sans-serif`, canvas.width);
...
export default {
  ...
  description: "Generate gambar fake call Android (WhatsApp Business style) dengan nama kontak, durasi, dan foto profil",
  category: "Maker",
  methods: ["GET", "POST"],
  ...
  async run(req, res) {
    ...
    try {
      ...
      const buffer = await generateFakeCallAndroid({ avatarUrl: avatar, name, duration });
      ...
      const durationMs = Date.now() - startTime;
      logger.error(`[FakeCallAndroid] Error after ${durationMs}ms: ${err.message}`);
      return res.status(500).json({
        status: false,
        message: err.message || "Gagal membuat gambar fake call Android"
      });
    }
  }
};
