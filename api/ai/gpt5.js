...
async function ssePost(jar, body) {
  const events = []
  const payload = JSON.stringify(body)

  const { data, headers } = await axios.post('https://chat.chatex.ai/api/chat', body, {
    headers: {
      ...baseHeaders(jar),
  ...
  return new Promise((resolve, reject) => {
    ...
    data.on('end', () => resolve({ headers, events }))
...
export default {
  ...
  paramsSchema: {
    teks: {
      type: "string",
      required: true,
      description: "Pertanyaan atau teks untuk Chatex AI",
    },
    //model: {
    //  type: "string",
    //  required: false,
    //  description: "Model AI (default: openai/gpt-5.4)",
    //},
  },
  ...
  async run(req, res) {
    try {
      ...
      const ai = await chatex(teks.trim(), model || "openai/gpt-5.4")
      ...
      res.status(500).json({
        status: false,
        message: err.message || "Chatex AI request failed",
      })
    }
  },
}
