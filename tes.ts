...
export async function bypassModJall(rawUrl: string): Promise<ModJallResult> {
    ...
    try {
        ...
        const directToken = (html1.match(/var\s+token\s*=\s*["\x27]([a-f0-9]{32})["\x27]/i) ||
            html1.match(/value=["\x27]([a-f0-9]{32})["\x27]/i))?.[1];
        ...
        if (!rayId || !alias) {
            throw new Error("Gagal mengekstrak ray_id atau alias dari halaman token.");
        }
        ...
        const finalToken = (htmlFinal.match(/var\s+token\s*=\s*["\x27]([a-f0-9]{32})["\x27]/i) ||
            htmlFinal.match(/value=["\x27]([a-f0-9]{32})["\x27]/i))?.[1];
        ...
        return {
            success: false,
            original_url: rawUrl,
            error: err.message || "Terjadi kesalahan saat memproses bypass.",
            execution_time_ms: Math.round(endTime - startTime)
        };
    }
}
