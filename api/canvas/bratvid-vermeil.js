/**
 * Bratvid Vermeil — video generator with word-by-word reveal
 * Uses @napi-rs/canvas + ffmpeg
 *
 * GET  /api/canvas/bratvid-vermeil?text=Watashi+wa+Verumei...
 * POST /api/canvas/bratvid-vermeil -d {"text":"Watashi wa Verumei..."}
 */
...
async function ensureFont() {
  ...
  try {
    ...
    if (fs.existsSync(LOCAL_FONT_PATH)) {
      console.warn(`[BratvidVermeil] Font fetch failed (${err.message}); using local fallback ${LOCAL_FONT_PATH}`);
...
/**
 * Resolve per-frame durations from tempo/timestamps/BPM
 */
function resolveDurations(frames, lyric) {
  ...
  if (Array.isArray(lyric.timestamps) && lyric.timestamps.length >= frames.length) {
    return frames.map((frame, i) => {
      ...
      if (!Number.isFinite(next) || !Number.isFinite(current)) {
        return Math.max(0.05, lyric.frameDuration);
      }
  ...
  if (Number.isFinite(lyric.bpm) && lyric.bpm > 0) {
    ...
    return frames.map(frame =>
      frame.isLastInLayer ? Math.max(0.05, lyric.lastFrameDuration) : beat
    );
  }
  return frames.map(frame =>
    frame.isLastInLayer
...
function redistributeDurations(frames, totalDuration) {
  const nRegular = frames.filter(f => !f.isLastInLayer).length;
  const nLast = frames.filter(f => f.isLastInLayer).length;

  // Last-in-layer frames get 1.5x weight for extra reading time
  const d = totalDuration / (nRegular + nLast * 1.5);
  frames.forEach(f => {
    f.duration = f.isLastInLayer ? Math.max(0.05, d * 1.5) : Math.max(0.05, d);
  });
  return frames;
}
...
async function generateBratVid(text, config) {
  ...
  let frames = buildRevealFrames(text, config);
  if (!frames.length) throw new Error("Teks kosong");

  // Redistribute durations if totalDuration is specified
  if (config.lyric.totalDuration) {
    frames = redistributeDurations(frames, config.lyric.totalDuration);
  }
  ...
  try {
    ...
    if (fs.existsSync(LOCAL_IMAGE_PATH)) {
      console.warn(`[BratvidVermeil] Image fetch failed (${err.message}); using local fallback ${LOCAL_IMAGE_PATH}`);
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "vermeil-bratvid-"));
  ...
  try {
    ...
    await encodeVideo(concatPath, outputPath, config);
...
export default {
  name: "Bratvid Vermeil",
  description: "Generate video Vermeil dengan teks muncul bertahap per kata — auto center, auto wrap, auto resize font. Support BPM/tempo/timestamps untuk sync durasi frame.",
  category: "Canvas",
  methods: ["GET", "POST"],
  ...
  paramsSchema: {
    text: {
      ...
      description: "Teks yang akan ditampilkan bertahap di video Vermeil"
    },
    duration: {
      type: "number", required: false,
      description: "Total durasi video dalam detik (default: otomatis dari frameDuration). Per-frame duration dihitung otomatis.",
      example: 10
    }
  },
  ...
  async run(req, res) {
    try {
      ...
      const config = {
        ...
        lyric: {
          ...VIDEO_CONFIG.lyric,
          totalDuration: duration ? Math.max(1, Number(duration)) : undefined
        }
      };
      ...
      res.setHeader("Content-Disposition", "inline; filename=\"bratvid-vermeil.mp4\"");
