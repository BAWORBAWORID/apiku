/**
 * AI Perplexity API - Powered by Perplexity
 * 
 * GET /ai/perplexity?teks=apa+itu+AI
 * POST /ai/perplexity (JSON: { teks: "apa itu AI" })
 * 
 * Result: JSON response sederhana
 * 
 * Author : Gienetic (Updated to Perplexity AI)
 * Category: AI
 */
...
export default {
  name: "AI Perplexity",
  description: "Akses Perplexity AI dengan response sederhana",
  category: "AI CHAT",
  methods: ["GET", "POST"],
  ...
  paramsSchema: {
    teks: {
      type: "string",
      required: true,
      default: "Jelaskan tentang artificial intelligence",
      description: "Pertanyaan atau teks yang ingin ditanyakan ke AI",
      example: "apa itu AI"
    }
  },
  ...
  async run(req, res) {
    try {
      ...
      logger.info(`[PERPLEXITY] Request | ip=${req.ip} | text="${text.substring(0, 50)}..."`)

      const result = await perplexity.ask(text)
      ...
      return res.status(200).json({
        statusCode: 200,
        status: true,
        message: "Perplexity AI berhasil memproses permintaan",
        data: {
      ...
      logger.error(`[PERPLEXITY] Error | ip=${req.ip} | error=${error.message}`)
