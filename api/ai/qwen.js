...
Author : ZenzzXD
Sementara gini dulu yak, kurangnya kalian bisa kreasikan sendiri awokawok
*/
...
async function qwenai(prompt, model = 'qwen3.7-plus') {
  ...
  return new Promise((resolve, reject) => {
    ...
    stream.data.on('end', () => resolve({ response: text.trim() }));
...
export default {
  ...
  paramsSchema: {
    teks: {
      type: "string",
      required: true,
      description: "Pertanyaan atau perintah untuk Qwen AI",
  ...
  async run(req, res) {
    ...
    try {
      const result = await qwenai(teks.trim(), "qwen3.7-plus");
      res.json({
        status: true,
        model: "qwen3.7-plus",
        input: teks.trim(),
      ...
      res.status(500).json({ status: false, message: err.message || "Qwen AI request failed" });
    }
  },
};
