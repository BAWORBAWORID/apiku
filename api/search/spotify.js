...
async function runQuery(term, hash, limit, auth) {
    const params = new URLSearchParams({
        ...
        variables: JSON.stringify({ searchTerm: term, offset: 0, limit: limit + 2, numberOfTopResults: 1, includeAudiobooks: false }),
...
async function spotifySearch(searchTerm, limit = 3) {
    ...
    try {
        ...
        logger.error(`Spotify Search Error: ${err.message}`)
        return []
    }
}
...
export default {
  ...
  paramsSchema: {
    query: {
      type: "string",
      required: true,
      description: "Kata kunci lagu/artis yang ingin dicari"
    },
    limit: {
      type: "number",
      required: false,
      description: "Jumlah hasil pencarian (default 3, maksimum 10)"
    }
  },
  ...
  async run(req, res) {
    try {
      ...
      const searchLimit = limit ? Math.min(10, Math.max(1, parseInt(limit))) : 3
      const result = await spotifySearch(query.trim(), searchLimit)
      ...
      console.error("Spotify Search API Error:", err.message)
      res.status(500).json({
        status: false,
        message: err.message || "Gagal mencari lagu di Spotify",
        timestamp: Date.now()
      })
    }
  },
